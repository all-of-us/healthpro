...
class WorkQueue
{
    ...
    public function generateTableRows($participants, $app)
    {
        ...
        foreach ($participants as $participant) {
            ...
            $row['ehrConsent'] = $this->displayStatus($participant->consentForElectronicHealthRecords, 'SUBMITTED', $participant->consentForElectronicHealthRecordsTime, true, 'INVALID');
    ...
    public function displayStatus($value, $successStatus, $time = null, $showNotCompleteText = false, $invalidStatus = null)
    {
        if ($value === $successStatus) {
            ...
            if ($showNotCompleteText) {
                $statusText = $value === $invalidStatus ? '(invalid)' : '(not completed)';
                return !empty($time) ? self::HTML_DANGER . ' ' . self::dateFromString($time, $this->app->getUserTimezone()) : self::HTML_DANGER . $statusText;
            }
